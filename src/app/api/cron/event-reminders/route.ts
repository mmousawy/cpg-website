...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: rsvpReminderEvents, error: rsvpEventsError } = await supabase
      .from('events')
      .select('id, title, slug, date, time, location, description, cover_image, created_at, image_blurhash, image_height, image_width, max_attendees, rsvp_count, attendee_reminder_sent_at, rsvp_reminder_sent_at, is_draft')
